...
| `hx-select` to trim the response | for navigation, unnecessary — the server renders the fragment shape. For a **form**, `data-gina-form-select` trims the answer. For a **popin**, [`data-gina-dialog-target`](/guides/popin#partial-swaps) — one plain selector picks the region out of the answer *and* names the slot it replaces in the open dialog; a miss falls back silently, announced in dev mode |
| `HX-Retarget` / `HX-Reswap` / `HX-Reselect` response headers | for a **form**, `X-Gina-Retarget` / `X-Gina-Reswap` / `X-Gina-Reselect`, read before `beforeswap` and honoured from the page's own origin only — [server-driven overrides](/guides/forms-and-validation#server-driven-overrides) |
