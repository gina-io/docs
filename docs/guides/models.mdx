...
[Couchbase ORM → `@options` annotations](/guides/couchbase-orm) for the full table.
...
configure a custom error page for `503` (or `5xx`) via `templates.json`'s
`_common.errorFiles`, it receives the cleaned-up `error` text in its template
data.
