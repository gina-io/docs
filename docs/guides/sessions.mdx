...
:::note `ttl` and `prefix` are constructor options here, not `connectors.json` fields
Unlike the SQLite, Redis, MongoDB and ScyllaDB stores — which read `ttl` (and,
for SQLite and Redis, `prefix`) straight from `connectors.json` — the Couchbase
stores read both from the options object you pass to the constructor. A `ttl` or
`prefix` key placed in `connectors.json` under a Couchbase connector is silently
ignored.

```js
store: new CouchbaseStore({ ttl: 86400, prefix: 'sess:' })
```
