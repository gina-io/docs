...
There is deliberately no configuration key for it, because it is not a
deployment-wide value — it belongs to one session, and is decided while that
session is being created. `express-session` already models this. Every session
carries its own `cookie` object and the framework never rewrites it, so
assigning `req.session.cookie.maxAge` in your login action is the whole
mechanism:
...
browser with no `Set-Cookie` at all — remembered or not. Measured on 0.6.31:
the user is bound and the record is written correctly, and the next request
still arrives anonymous. Fixed in 0.6.32. On earlier releases, confirm that a
plain login survives one page load before adding a remember-me box on top of
it.
:::
