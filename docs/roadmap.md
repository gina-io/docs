...
| ✅ | **`Content-Security-Policy` (Phase 2 — static directives)** — Opt-in plugin `gina.plugins.Csp({ directives, reportOnly })` emits the Content-Security-Policy response header on every response (#HDR5). **Opens Phase 2** of the security-headers track. v0 ships static directives only — per-response nonce wiring requires template-render integration and defers to a future CSP-aware view-layer plugin. Strict whitelist of 27 CSP Level 3 standard directives; unknown directive names throw at factory call time (CSP typos are silent in the browser — fail-fast catches them). Value parsing: array (joined with space), pre-formatted string, `true` (boolean-only directives + `sandbox`), or `false` (omit). `reportOnly: true` switches the header name to `Content-Security-Policy-Report-Only` for non-enforcing migration testing. `directives` is required (no sensible cross-bundle default). Idempotent — first-writer-wins. 92 unit tests. See the [Content-Security-Policy guide](/guides/csp) for the full reference. | `0.4.0-alpha` | 2026-05-17 |
