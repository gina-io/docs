...
| 📋 | **`SecurityHeaders` combined wrapper (Phase 2 — closes #HDR)** — Opt-in plugin `gina.plugins.SecurityHeaders({...})` composes HDR1-7 + HDR5 + HDR6/13/14 in a single mount. One settings.json block (`securityHeaders.*`) with per-header sub-configs. Individual plugins remain mountable independently as power-user escape hatches. Mirrors helmet's `helmet()` orchestrator. **Closes Phase 2.** | `0.4.0-alpha` | 2026-05-17 |
