...
| 📋 | **`Cross-Origin-Resource-Policy` (CORP) middleware (Phase 2)** — Opt-in plugin `gina.plugins.Corp({ value })`. Settings: `corp.value` is one of the W3C HTML spec tokens (`"same-origin"`, `"same-site"`, `"cross-origin"`); default `"same-origin"` matches helmet. Most restrictive practical default; restricts which other origins can fetch this resource. Use `"cross-origin"` for resources intended to be publicly embeddable (CDN fonts, images). | `0.4.0-alpha` | 2026-05-17 |
