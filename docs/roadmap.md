...
| ✅ | **Phase 1.5 — `HidePoweredBy` (#HDR8)** — Opt-in plugin `gina.plugins.HidePoweredBy()` removes the `X-Powered-By` response header that gina emits by default at `core/server.js:2425` (plus `core/template/conf/env.json > response.header`). Reduces the attacker reconnaissance surface — the server stack identity no longer leaks via the response header. **Different SHAPE from #HDR1-#HDR7**: REMOVE (`res.removeHeader`) not SET. No tunable options today (mirrors helmet's no-opts shape). **Engine-asymmetric effectiveness**: works on the Express engine (`server.js:2425` setHeader fires in early framework middleware before user `app.use()` mounts, so `removeHeader` succeeds); does NOT work on the Isaac engine — `server.isaac.js` has 15+ direct `response.writeHead({ 'X-Powered-By': ... })` call sites that bypass the setHeader/removeHeader interface. README documents the gap. Idempotent — `removeHeader` is a no-op when the header is absent. 37 unit tests. See the [Hide X-Powered-By section](/guides/security-headers#hide-x-powered-by-hdr8) for the full reference. **Opens Phase 1.5 (helmet-parity gap-fill).** | `0.3.16-alpha` | 2026-05-17 |
