...
single-file form** (fourth bullet). The three `lib/archiver` fixes and the fluent-handle fix (fifth bullet) are server-side
(`lib/archiver` is not part of the browser bundle), so they arrive with a bundle
