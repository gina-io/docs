...
single-file form** (fourth bullet). The three `lib/archiver` fixes, the two fluent-handle fixes (fifth and sixth bullets) and the Inspector payload fix (seventh bullet) are server-side
(neither `lib/archiver` nor the controller is part of the browser bundle), so they arrive with a bundle
**restart**; the two autofill fixes (first bullet) are browser-bundled, so also
**rebuild each bundle** to pick it up.

- **Autofilled forms enable their submit control.** A browser or password-manager fill produced no keystroke and, on Chrome, no `input`/`change` until a later gesture, so the live check never re-ran: the submit control stayed gated on a visibly complete form, and an autofilled invalid value showed no error. Gina's stylesheet now gives every autofilled control a 1ms keyframe named `gina-autofill-start`, and the validator treats the resulting `animationstart` as that field's live check. A value the browser still withholds from script (Chrome, until your first gesture) no longer counts against the gate and renders no false "required" error, while the submit-time check keeps refusing it until it is released. Additive unless your own CSS sets `animation-name` on `:-webkit-autofill` / `:autofill` inputs, which would replace the hook — keep `gina-autofill-start` there. Submitting while the value is still withheld now renders the honest `isRequiredAutofill` label (default *Filled in by your browser: click the field to confirm it*) instead of the plain required one and marks the control `data-gina-form-autofill-withheld="true"`; a catalog that translates `isRequired` but not the new key keeps its own wording for it, so a localized page changes nothing until you translate `isRequiredAutofill`.
- **`lib.archiver` calls no longer interfere with each other.** Every `compress()` and `decompress()` completion used to be signalled by emitting one fixed event name on the library's process-wide singleton, so two calls overlapping in one process — two queue consumers, or a request handler beside a worker — both received whichever run finished first: the second caller was released early with the *first* run's archive path, and its own archive was left at 0 bytes. The array form also kept its output stream in an implicit global, so finishing one run closed another's stream. Each call now settles **exactly once** on its own channel for its own `onComplete` listeners and trailing callback; a listener attached after the run has already settled is still delivered; and the documented `archiver-<method>#complete` event is still emitted on the singleton for observers, so code listening there keeps working. If you had serialised calls behind a process-wide mutex to work around this, the mutex can go once every tier runs this version. **Errors on the streams the library opens itself now reach your callback too:** an unreadable input used to either crash the process on an unhandled stream error or hang the run forever — leaving a corrupt partial archive on disk — depending on timing; an unwritable target crashed both the array and directory forms. All of them now arrive at `onComplete` as the stream's error (`err.code === 'EACCES'` and friends). The synchronous error paths (a missing source, a source that is neither a file nor a directory) reach `onComplete` instead of firing before the handle is returned, and both promisify shapes now work: a trailing callback with options lacking `method`, and `compress(src, target, cb)` with options omitted. The first of those was an indefinite hang, not an error: the completion listener was registered from `options.method` before the option defaults were merged, so a trailing-callback caller that did not set `method` waited on `archiver-undefined#complete` while the run emitted `archiver-gzip#complete`, and its callback never fired. The fluent handle form was never affected.
...
- **The single-file form of `compress()` now writes a real zip, and `decompress()` reads every form back.** It used to pipe the file through a gzip codec and write the result under the `.zip` name — `unzip` and `decompress()` both rejected it — and `method: 'br'` / `'deflate'` were accepted and silently produced that same gzip. The archive path is unchanged (`<target>/<name>.zip`, `<name>` defaulting to the file's basename) and now holds one entry named by the basename. **Action, only if you gunzipped the single-file output directly:** read it as a zip instead (`decompress()`, or any zip tool). `method` is validated against `gzip` alone — it names the `archiver-gzip#complete` event and selects no codec — so `br` and `deflate` now throw synchronously. The same branch also skipped any input whose path carried a dot-prefixed segment (`.env`, or a file under `~/.config/`), reporting success with the *input* path as the archive while leaving an empty `<name>.zip` and two leaked descriptors; such a file now archives like any other, as the directory form already did.
- **Fluent `.onComplete()` handles deliver per call.** `self.query(options, data).onComplete(cb)` used to register on the controller instance's shared emitter after clearing every previous `query#complete` listener, so a concurrent fluent query on the same controller evicted the first one — its callback never fired, the survivor could receive the other call's payload, and the evicted response was emitted to nobody, on both transports, with no log line; `self.store(target).onComplete(cb)` registered with `on()` and never removed it, so a second `store()` on the same controller re-invoked every earlier callback with the later result; and the synchronous failure paths of `query()` (missing host, open circuit, unreadable certificate) returned a boolean instead of the handle, so chaining `.onComplete` threw `TypeError`. Each fluent call now has its own delivery channel and `query()` returns the handle on every path: registrations chain and all fire, a late registration fires on the next tick, a synchronous failure reaches `cb(err)` on the next tick, and the delivered shapes are unchanged (`cb(false, data)`, `cb(data)` for a known non-2xx status, the `{status, error}` wrap for transport failures). Two events fire less often as a result: `query#complete` only when nothing consumed the outcome (no callback, no `.onComplete()`), and `'uploaded'` no longer for the fluent `store()` form — neither had a documented or in-tree listener. Nothing to change in application code; the callback and promisified forms are untouched.
- **A `query()` issued while rendering from another required controller now settles.** The guard that refuses `self.query()` once a second render has been entered on the request (`requireController` shares the caller's options object, so the required controller's render deepens the same `renderingStack`) returned the literal `false` and delivered nothing: in the callback form the callback never fired, and the fluent `self.query(options, data).onComplete(cb)` chain threw `TypeError` on the boolean. The refusal now travels like every other synchronous failure: the callback receives an `Error` whose `code` is `NESTED_RENDER` — in-line in the callback form, on the next tick through the handle, wrapped as `{status: 500, error}` — the handle comes back, and the upstream is still never contacted. The guard itself is unchanged, so a query in that state still fires no request. **Visible only in that nested state:** a callback that used to stay silent now runs with the error — an `if (err) return self.throwError(...)` there is absorbed by `throwError`'s own nested-render guard, but any logging or metrics in your error branch fire once per refused call, which is the honest reading.
- **The dev Inspector's `window.__ginaData` payload is built per request, never compiled.** With `server.cache.enable` on and the Inspector block injected (automatic in dev; in production only when a controller calls `self.render(data, true)`), the payload — a snapshot of `data.page`: the controller's data, the session card, the environment — used to be baked into the swig compiled template and the persisted `.gina-layout-cache` file, so every later render of that view, every other view compiled after it, and every later process served the **first** request's page data, and `/_gina/reveal` plus the Inspector stream froze with it; the hidden inputs that carry a popin's data to the Inspector were baked the same way. Both are now spliced into the executed HTML after the template runs, a cache hit refreshes the Inspector sinks, and a layout-cache file left by an earlier release is healed on the next compile. Nothing to change: restart the bundle; deleting `.gina-layout-cache` under the bundle's templates root is optional, never required. The script still lands ahead of the statusbar, so anything of yours that reads `window.__ginaData` after it is unaffected.

## 0.6.25 → 0.6.26

**Additive — no action required.** Every fix in this release is server-side; the
browser bundle is byte-identical to `0.6.25`, so a bundle **restart** is the
whole pickup and no rebuild is needed.

- **An error response no longer crashes on a non-numeric payload `status`.** On the error-render path, a top-level `status` in the rendered payload was stamped straight into the HTTP response with no validation. A page whose own vocabulary includes a field named `status` — `{ status: "draft" }` rendered alongside an error, say — therefore threw `ERR_HTTP_INVALID_STATUS_CODE`, and the intended error page was replaced by an unhandled 500. Because it only fires on the error path, the failure was invisible on every successful render and appeared exactly when the error page was needed. Every status the error path resolves is now validated against node's own rule — an integer from 100 to 999 — and degrades to a 500 error page when it does not qualify. A numeric status still sets the HTTP status exactly as documented, so a route already passing a valid code is unchanged. This also closes the same hole for an explicitly passed three-digit-but-invalid code such as `"099"`, which node rejects and which previously threw as well.
- **Error responses now carry the content-type they declare.** Two branches of the error path — the Internet Explorer override, and the branch taken when a request arrives with **no `user-agent` header at all**, which is the common shape for machine callers — passed the header *name* as node's status-message argument instead of a headers object. Node read the MIME string character by character: over HTTP/1.1 the response came back with roughly ten to thirty junk headers named `0`, `1`, `2`…, a status line whose reason phrase read `content-type`, and no content-type header at all; over HTTP/2 the headers were dropped entirely and a status-message warning was emitted. Nothing threw, so the malformed response was completely silent. Both branches now pass a headers object. **The plain-text branch additionally declares your bundle's encoding as its charset, and that half is user-visible wherever your error copy is not plain ASCII** — the error body is JSON written as UTF-8, so a charset-less `text/plain` invited the client to guess a single-byte encoding and render accented characters or non-Latin scripts in a visitor-facing message as mojibake.
- **Staged upload parts stranded by a previous process are reclaimed at boot.** The `upload.autoTmpCleanupTimeout` deletion timer is per-upload and lives in the process that created it, so a restart stranded every staged part it was holding; and a part that never reached `self.store()` — a request failing after the multipart parse, or an application consuming the staged file some other way — had no reclaim path at all. On deployments that restart more often than the configured timeout, server-generated `.part` files accumulated indefinitely in the configured landing directories. **When the timer is armed**, server init now sweeps each configured landing directory (the global default plus every group `path`, deduplicated) for staged-shaped `.part` files older than the configured timeout, **floored at one hour** so an in-flight upload from a sibling live process sharing the staging directory is never touched. The sweep is deliberately conservative: exact server-generated name shape only — a client-named orphan predating the opaque staging names is indistinguishable from an application file and is left alone — regular files only, symlinks never followed, non-recursive, and never blocking boot. **A disabled `autoTmpCleanupTimeout` — the shipped default — still arms no sweep**, so if you rely on the documented "removing staged files is the operator's job" contract, nothing changes for you. Applications may also remove `req.files[].path` themselves; the publish path has always tolerated a vanished source, and that tolerance is now a documented contract rather than an accident.
- **Job records stranded by a process death are reclaimed.** The deferred function of an async job lives only in the process that created it — a closure cannot be serialised — so a record left `running`, or `pending` past its scheduled retry, by a crash or an unclean restart could never settle. On a durable store its `expiresAt` stayed `null` forever, the TTL sweep's own guard excluded it by that guard's first conjunct, listings reported phantom running work, and the documented polling pattern never terminated. The sweep is now preceded by an **orphan-reclaim pass on durable stores**: a non-terminal record whose last transition — or whose scheduled retry — is older than `jobs.orphanTimeout` is marked `failed` with error name `JobOrphanedError`, after which normal `ttl` retention removes it on schedule. The new knob lives in `app.json` under `jobs`: **`orphanTimeout`, in seconds, default `86400` (24h), `0` or `false` disables the pass, positive values are floored at 60.** The built-in memory store is exempt, since an in-process store cannot hold another process's orphans. A record reclaimed while its origin was merely slow self-corrects on the origin's next write, and **no completion webhook fires for a reclaim** — concurrent reclaimers and a self-healing job would otherwise produce contradictory notifications, and the record itself carries the truth. If you poll `jobStatus(id)`, a previously-hanging poll now terminates on a `failed` record carrying `JobOrphanedError`; handle that error name if you want to distinguish a reclaimed job from one your own code failed.
...
- **New FormValidator rules `isIban` and `isBic`.** IBAN validation per ISO 13616 (shape, official per-country length for 87 registered countries, and the ISO 7064 MOD 97-10 checksum) with a tolerant read — case and space/hyphen separators are normalized for validation only; the stored value is never mutated — plus BIC validation per ISO 9362 (8/11-character shape, case-insensitive). Client + server like every built-in rule; empty values stay adjudicated by `isRequired` alone. See the [Validation rules reference](/reference/validation-rules#isiban).
- **Idempotency keys (opt-in).** `Idempotency-Key` request deduplication at the router band, per the IETF draft: arm `server.idempotency` in settings.json (a declared kv namespace + `keyField`) and opt routes in with routing.json top-level `idempotency: true` (or `{ "required": true }`). Retried mutations replay the recorded first response (`Idempotency-Replayed: true`); 409 + Retry-After while the original is in flight; 422 when a key is reused with a different payload. Principal-scoped — anonymous callers are skipped. Dormant unless enabled. See the [Idempotency keys guide](/guides/idempotency).
- **New `self.renderXML(xmlContent, contentType)`.** First-class XML responses, the outbound counterpart to verbatim XML request handling. Send a pre-serialised document — gina builds no XML any more than it parses any — and the delegate handles the wire: `application/xml` by default with your bundle's charset ([RFC 7303](https://www.rfc-editor.org/rfc/rfc7303) §4.1 recommends it over `text/xml`), an optional second argument for the `+xml` suffix family (`application/soap+xml`, `application/atom+xml`, a vendor tree), HEAD suppression with a byte-accurate `content-length`, the HTTP/2 pseudo-header status with upstream headers folded into the frame, and opt-in trailers. The status comes from `res.statusCode` rather than the payload — an XML string has no `{ status }` envelope — so set it upstream or use `self.throwError()`. A route using idempotency keys records its envelope here too. See [Controllers · `self.renderXML()`](/guides/controller#selfrenderxmlxmlcontent-contenttype).
- **Message-schema validation seam (`param.messageValidator`).** Routes can run the raw request body through an application-supplied validator before the controller action: declare `"messageValidator": "<name>"` in a route's `param` block and ship a factory module at `message-validators/<name>.js` — the factory runs once at boot (compile your XSD set or open your sidecar pool there; a broken module refuses the boot instead of silently skipping validation), and the returned `validate(document, req)` — sync or async — receives the verbatim body string. Refusals are fail-closed: 422 by default with your `errors` array on the body, 400 for an unparseable document, 503 + `Retry-After` when the checker itself is down, 500 on a thrown error. Gina ships no schema engine — the validator is yours; the hook, ordering (after auth/rate-limit/idempotency, before DTO field validation) and refusal shape are the framework's. Routes without the declaration are byte-identical. See the [Message validation guide](/guides/message-validation).
- **XML request bodies now reach your action verbatim.** A POST, PUT or PATCH whose `Content-Type` is `application/xml`, `text/xml` or any `application/*+xml` type (SOAP, Atom, RSS, vendor trees) is no longer run through the form-encoded decode path. `req.body` is the exact document as a **string** — the same value `req.rawBody` carries — and you parse it with the library of your choice; gina never parses XML itself. Previously such a body was silently destroyed rather than rejected: the form decoder splits on `&` and then `=` and keeps only the first pair, so an XML declaration alone collapsed the whole document to a single bogus key, which then replaced `req.body`. **The only visible shape change is on that broken path** — for an XML request `req.body` is now a string instead of that truncated object, and `req.post` / `req.put` / `req.patch` is an empty object; no working code could have depended on the previous value. urlencoded, JSON and multipart bodies are unaffected. See [Controllers · XML request bodies](/guides/controller#xml-request-bodies).
- **The `query()` basic-auth option now works on HTTP/2 — and no longer leaks.** `options.auth` (`"user:password"`) was consumed by node's own `request()` on HTTP/1.x only; on the HTTP/2 path it was forwarded into the outgoing header block as a literal `auth:` header while no `Authorization` header was ever minted — the credential travelled in a nonstandard header and authentication silently never happened. The option is now minted into `Authorization: Basic <base64>` before dispatch on both transports through one code path, and the option itself is deleted pre-dispatch, so an empty or malformed credential cannot travel either. A caller-supplied `authorization` header always wins, matching node's HTTP/1.x behaviour. If anything downstream relied on receiving the literal `auth:` header on HTTP/2, that was the defect, not a contract. Server-side only: restart, no rebuild — for this fix alone; this release as a whole does change the browser bundle (see the release notes).
- **Inspector data serialisation also escapes template delimiters.** Server data serialised into the Inspector's inline `<script>` blocks (`__ginaData`, `__ginaLogs`) is spliced into the layout before swig compiles it, so `{{ }}`, `{% %}` and `{# #}` inside a stored value were evaluated as template source. The shared inline-script helper now escapes `{` and `}` inside JSON string literals as well as `<`; values parse back identical, structural braces are untouched, and the nunjucks renderer (which injects after rendering) was never affected. Same reachability as the 0.6.23 hardening — only pages that emit the Inspector block. Server-side only: restart, no rebuild.
