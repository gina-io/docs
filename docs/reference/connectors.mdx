...
| [ScyllaDB / Cassandra](/guides/scylladb-orm) | Wide-column | High-throughput application data | ✅ Yes — distributed by design | ✅ Available |
| [DuckDB](/guides/duckdb-analytics) | Embedded analytical (columnar / OLAP) | In-process analytics & reporting | ❌ No — embedded file; single writer across processes (read-only sharing supported) | ✅ Available (v0.6.1) |
...
`deleteMany`) — see the [MongoDB ORM guide](/guides/connectors-mongodb).
...
See the [Couchbase ORM guide](../guides/couchbase-orm) for the bucket-handle wiring and
...
[MongoDB connector guide](../guides/connectors-mongodb).
...
takes precedence over the `connectors.json` entry. See the
[ScyllaDB ORM guide](../guides/scylladb-orm).
...
into private internals. See [Accessing the underlying SDK Cluster](/guides/couchbase-orm#accessing-the-underlying-sdk-cluster)
and [Accessing the underlying MongoClient](/guides/connectors-mongodb#accessing-the-underlying-mongoclient).
