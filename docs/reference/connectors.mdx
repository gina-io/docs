...
The [`connector:list`](/cli/connector#connectorlist) command lists every connector across your projects with driver install status, and [`connector:add`](/cli/connector#connectoradd) writes new entries to `shared/config/connectors.json` or a bundle-scoped `connectors.json` with the correct shape.
:::
...
- [`connector:list`](/cli/connector#connectorlist) reports the pin in the driver-info column and raises a `[ !! ]` warning when two bundles pin the same driver at different versions (npm resolves one version per project, so the first `npm install` wins).
- [`connector:add`](/cli/connector#connectoradd) uses the pin to print the exact `npm install <pkg>@"<range>"` command after writing the entry. Set it with `--driver-version=<range>`.
