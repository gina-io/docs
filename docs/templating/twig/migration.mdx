...
flowchart TD
  ...
  C -->|No| D{Uses dynamic from?}
  D -->|Yes| H[from path must be a string literal - inline or import statically]
...
swig-twig's `date` filter uses the PHP-style format tokens documented under [native swig's `date` filter](../swig/filters) (shared with `@rhinostone/swig-core`). Most format strings port — `"Y-m-d H:i:s"`, `"U"`, etc. — but locale-aware names (weekday names, month names in non-English locales) are not plumbed through. If you rely on locale switching, pre-format on the controller side and pass the string into the template.
...
See [Extending Swig](../swig/extending) for the registration contract.

### Template inheritance

Two semantic differences from upstream:

- **Dynamic parent paths resolve on the async render path only.** `{% extends layout_var %}` works under `renderFile(path, locals, cb)` with `loader.async === true`; the synchronous path requires a string-literal parent (dispatch the chosen parent in the caller). See [Non-Goals — dynamic extends](./non-goals#dynamic-extends) and [Async loaders](../swig/loaders#async-loaders).
...
- **Isolation.** Tags, filters, and extensions registered on one instance are invisible to other instances — same guarantee as native swig. See [Security — design invariants](../swig/security#security-model).
- **Cache semantics.** Compiled functions are keyed by the loader-resolved filename. See [Loaders](../swig/loaders) for the contract.
