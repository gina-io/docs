...
`@rhinostone/swig-twig` does not ship a pre-built browser bundle. Bundle `@rhinostone/swig-twig` through your own toolchain (esbuild, Vite, Webpack, Rollup) and follow the shared Swig [Browser Usage](../swig/browser) recipe — memory loader only, no `fs` access, autoescape + CVE-2023-25345 guards inherited from `@rhinostone/swig-core`.
