...
Available since `v2.2.0`, the cb form of [`renderFile(path, locals, cb)`](./api#renderfile) and [`compileFile(path, { codegenMode: 'async' }, cb)`](./api#async-codegen) closes the gap. Set `loader.async === true` on the loader object and the dispatch automatically routes through the async-codegen path: pre-walk the template's dependency graph asynchronously through the user loader's `load(id, cb)` arm, populate an in-memory map keyed by resolved path, then run the existing sync pipeline against a memory wrapper for the duration of the call.
...
For users still on `@rhinostone/swig@2.1.0`, the `renderFileAsync(path, locals, cb)` / `compileFileAsync(path, options, cb)` methods cover the same async-loader case. They share the pre-walker mechanism and the static-path requirement above. Both methods are **soft-deprecated in `v2.2.0`** (JSDoc-only — no runtime warning) and will be removed in `v3.0`; new code should target the `renderFile` / `compileFile` cb path described above.
