...
Filters marked ✅ are not re-escaped by the autoescape pass — either flagged `.safe` (output trusted as-is), or, for `escape`/`e`, because the HTML escape is idempotent. See [Security](./security#what-the-guards-protect-against) for the model.
...
`escape` / `e` show ✅ because their output isn't double-escaped (the HTML escape is idempotent), not because they're flagged `.safe`. `url_decode` is **not** safe — its output is autoescaped, since decoded content can contain HTML.
...
Not flagged `.safe`, but its output is not double-escaped: the HTML escape is idempotent, so the autoescape pass leaves already-escaped output unchanged.
...
Marked **safe** — the output HTML-escapes `<`, `>`, `&`, and `'` (as `\u00xx`), so it is valid JSON that is safe to embed directly inside a `<script>` block.
...
`decodeURIComponent`. **Not** marked safe — its output is autoescaped, because decoded content can contain HTML-significant characters.
