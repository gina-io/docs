...
sidebar_position: 1
description: Swig is the Jinja2/Django-inspired template engine that Gina consumes via npm. This section documents @rhinostone/swig directly — syntax, built-in tags and filters, the API, loaders, the CLI, and the browser build.
level: beginner
prereqs:
...
Gina consumes [`@rhinostone/swig`](https://github.com/gina-io/swig) as a regular npm dependency, declared at a semver floor in the framework's `package.json`. This section documents `@rhinostone/swig` directly — installing it standalone, the template syntax, the full API, and how to extend it with custom tags, filters, and loaders.

## About this fork

The upstream project ([`paularmstrong/swig`](https://github.com/paularmstrong/swig)) is abandoned. `@rhinostone/swig` is the maintained successor that Gina depends on. Security patches (including [CVE-2023-25345](./security)) land here first; Gina picks them up on its next semver-allowed npm update.

- **Package:** `@rhinostone/swig` on npm
- **License:** MIT
- **Version:** `2.2.0` — the current recommended version for production use.
...
Set `swig.package: "@rhinostone/swig-twig"` to use the [Twig-syntax frontend](../twig) instead. The API is drop-in — `self.render(data)` still works — but templates must be written in Twig syntax, not swig. See the [Twig Frontend](../twig) section for install details.
