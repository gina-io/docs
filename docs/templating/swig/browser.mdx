...
- **Don't prime the cache with `swig.run(tpl, {}, path)` or by writing `swig.cache[key]` directly.** Earlier versions of this page recommended both. They store the raw compiled function — not the call shape `include` lookups use — so an included partial that touches variables or filters crashes, and an extends parent primed this way silently renders an empty page. Use [`register`](./api#register) / [`registerBundle`](./api#registerbundle) (v2.8.0) instead.
