...
Throws when the instance has caching disabled (`cache: false`) — the registry is the template cache.
...
Register a map of pre-compiled template functions in one call — the shape a [`swig compile --recursive`](./cli#aot-bundle--compile-a-directory) bundle exports. Added in v2.8.0. Bundles emitted with `--register` call this themselves when a global `swig` is present.
