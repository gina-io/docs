...
## Content negotiation (`negotiate: true`)

:::warning Not supported on nunjucks yet
A route declaring [`negotiate: true`](/guides/routing#content-negotiation) on a nunjucks bundle
currently answers a fragment request (`X-Gina-Navigate: fragment`) with the **full page** — the
layoutless flag only filters the asset list on this engine, and native `{% extends %}` resolves
the real layout regardless. [SPA navigation](/guides/client-navigation) would swap an entire
nested page into the region. Avoid `negotiate: true` on nunjucks `{% extends %}` routes until
this is resolved; swig — the default engine — is the fully working path.
:::
