...
- You need full parity with the swig rendering path — a few integrations (Early Hints 103 auto-send, static HTML cache, `setResources` / `<gina>` layout placeholders) are not yet ported. See [Deferred features](#deferred-features) below.
...
| Gina's built-in filter registry (`getUrl`, `getWebroot`, `length`, `nl2br`, `addHours`, `addDays`, `addYears`) | **Shipped in `0.3.7-alpha.4`** | — |
