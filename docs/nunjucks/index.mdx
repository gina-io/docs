...
| --- | --- | --- |
| Inspector `__ginaData` dev payload | **Deferred** | Inspector Data tab won't show payloads for nunjucks pages. |
| HTTP/2 `stream.respond()` direct path | **Deferred** | Uses `res.writeHead()` + `res.end()` through the HTTP/1.1 compat layer. |
...
| Error-page template routing | **Deferred** | Thrown errors render via the swig path (the framework error template is currently swig-only). |

Each is a focused follow-up session when the gap bites.
...
- `0.3.7-alpha.3` (2026-04-22) — `lib/nunjucks-resolver` primitive shipped in commit `63e7e569f`.
- `0.3.7-alpha.3` (2026-04-22) — `controller.render-nunjucks.js` MVP + dispatch shipped in commit `0980a00b7`.
