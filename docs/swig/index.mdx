...
Set `swig.package: "@rhinostone/swig-twig"` to use the [Twig-syntax frontend](/swig/twig) instead. The API is drop-in — `self.render(data)` still works — but templates must be written in Twig syntax, not swig. The framework floor for the Twig frontend is `2.0.0-alpha.8`.
