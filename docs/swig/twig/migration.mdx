...
If you rely on `css` or `html_attr`, register a custom filter and mark it `.safe = true`:

```js
twig.setFilter('css_escape', function (input) { /* your CSS-safe escape */ });
twig.filters.css_escape.safe = true;
```
