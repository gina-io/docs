...
Gina sets a `_scope` property on every entity from the connector entry's `scope`, or else `NODE_SCOPE` — that is the `local` / `staging` / `production` **deployment** axis: a per-process setting, never a per-request value. It structurally cannot carry a per-request user id. (The SQLite connector used here has no `$scope` placeholder at all; on Couchbase, where it exists, it is filled with that same per-process value on every query.) Per-user isolation is always what you see above: a bound argument. See [Models & entities](/guides/models) for the full scope story per connector.
:::
...
In a container, prefer `GINA_LOG_STDOUT=true` — same JSON, and it also skips the log-tail transport that has no listener inside a container. From 0.6.30 `gina-container` applies that preset itself when the variable is unset, so the explicit `GINA_LOG_STDOUT=true` in the examples below documents the default rather than adding a requirement (`GINA_LOG_STDOUT=false` opts out). The [Logging guide](/guides/logging) covers levels, files, and the resolution order.
...
| What | Where |
|---|---|
| **Make this project interactive** — rule-bound forms, a validation rule of your own, a live streaming counter, navigation without reloads | [Make It Interactive](/tutorials/make-it-interactive) |
