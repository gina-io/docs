...
Gina stores the database file at `~/.gina/shortener.sqlite` — that is `<gina home>/<database>.sqlite`, outside your project and not under version control. Add a `"file"` key if you would rather keep the database somewhere else:
...
- **`stats` before `redirect`** — good practice: put the more specific two-segment path (`/:slug/stats`) before the one-segment wildcard (`/:slug`). These two cannot match the same URL — their segment counts differ — but the router tests routes in declaration order and the first match wins, so where two patterns can overlap, the order decides.
...
| What | Where |
|---|---|
| **Take this project to production** — accounts, per-user data, HTTP/2, containers | [Going to Production](/tutorials/going-to-production) |
