...
The data passed to `self.render({ link: link })` is available as `page.data` in every [Swig](/views/swig) template. The `{% set data = page.data %}` alias at the top is a Gina convention (matches the boilerplate) that keeps template expressions concise.
