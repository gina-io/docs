...
Gina stores the database file under the per-version gina home at `~/.gina/<version>/shortener.sqlite` — that is `<gina home>/<database>.sqlite`, outside your project and not under version control. Add a `"file"` key if you would rather keep the database somewhere else:
